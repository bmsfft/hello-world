...
void send_from_file(
    ...
    const std::string &filetx,
    int num_channels
){

    std::vector<std::complex<short> *> buffs(num_channels, &buff.front());
    std::ifstream infile(filetx.c_str(), std::ifstream::binary);
    ...
    while(not stop_signal_called){
        ...
        infile.read((char*)&buff.front(), buff.size()*sizeof(std::complex<short>));
        size_t num_tx_samps = size_t(infile.gcount()/sizeof(std::complex<short>));
...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    std::string tx_args, filetx, tx_ant, tx_subdev, ref, otw, tx_channels, wirefmt;
    double tx_rate, tx_freq, tx_gain, tx_bw, delay;
    size_t spb;
    ...
    desc.add_options()
		...
		("filetx", po::value<std::string>(&filetx)->default_value("usrp_samples6.dat"), "name of the file to read binary samples from")
		("file", po::value<std::string>(&file)->default_value("rx_samples.dat"), "name of the file to write binary samples to")
        ...
        ("tx-rate", po::value<double>(&tx_rate)->default_value(8e6), "rate of transmit outgoing samples")
        ("rx-rate", po::value<double>(&rx_rate)->default_value(8e6), "rate of receive incoming samples")
        ("tx-freq", po::value<double>(&tx_freq)->default_value(72e6), "transmit RF center frequency in Hz")
        ("rx-freq", po::value<double>(&rx_freq)->default_value(72e6), "receive RF center frequency in Hz")
        ("tx-gain", po::value<double>(&tx_gain)->default_value(40), "gain for the transmit RF chain")
        ("rx-gain", po::value<double>(&rx_gain)->default_value(40), "gain for the receive RF chain")
		...
		("tx-subdev", po::value<std::string>(&tx_subdev)->default_value("A:A"), "transmit subdevice specification")
		("rx-subdev", po::value<std::string>(&rx_subdev)->default_value("A:A"), "receive subdevice specification")
        ("tx-bw", po::value<double>(&tx_bw)->default_value(4e6), "analog transmit filter bandwidth in Hz")
        ("rx-bw", po::value<double>(&rx_bw)->default_value(4e6), "analog receive filter bandwidth in Hz")
    ...
    if (vm.count("help")){
        std::cout << boost::format("UHD TXRX Loopback to File Test %s") % desc << std::endl;
        return ~0;
    }
    ...
    //allocate a buffer which we re-use for each tx and rx channel
    ...
    //start trasmitter thread. std::cout << "Thread Spawned" << std::endl;
    boost::thread_group transmit_thread;
    transmit_thread.create_thread(boost::bind(&send_from_file, buff, tx_stream, md, filetx, num_channels));
